...
@HiltViewModel
class FriendWorkoutsViewModel @Inject constructor(
    ...
    fun loadFriendWorkouts(friendsname: String, startDate: LocalDate, endDate: LocalDate) {
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.value = when (result) {
                    is Result.Success -> {
                        Log.d("FriendWorkouts", "Loaded ${result.data.size} workouts for friend $friendsname")
                        UiState.Success(result.data)
                    }
                    is Result.Error -> {
                        Log.e("FriendWorkouts", "Error loading workouts: ${result.error}")
                        UiState.Error(ErrorUiMapper.toMessage(result.error))
                    }
                }
            } catch (e: Exception) {
                Log.e("FriendWorkouts", "Exception loading workouts", e)
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _workoutDetailsState.value = when (result) {
                    is Result.Success -> {
                        Log.d("FriendWorkouts", "Loaded workout details for $workoutId")
                        UiState.Success(result.data)
                    }
                    is Result.Error -> {
                        Log.e("FriendWorkouts", "Error loading workout details: ${result.error}")
                        UiState.Error(ErrorUiMapper.toMessage(result.error))
                    }
                }
            } catch (e: Exception) {
                Log.e("FriendWorkouts", "Exception loading workout details", e)
